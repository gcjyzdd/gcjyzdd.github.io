...
### Cuda: global function VS device function
...
Notes: Just as an addendum, `__global__` functions can also be called from the device using CUDA kernel semantics (`<<<...>>>`) if you are using `dynamic parallelism` - that requires CUDA 5.0 and compute capability 3.5 or higher
...
```sh
./samples_camera_gmsl --write-file=dil.avi
```

To take screenshots of the camera view, press key `s`.

### Opencv Cmake Errors

Cannot find lib `opencv_dep_cudart`.

**Solution**: 

Open the following file with root right:

```sh
/usr/local/share/OpenCVOpenCVModules-release.cmake
```

search and replace `opencv_dep_cudart;` with empty:

```sh
%s/opencv_dep_cudart;//g
wq
```

Check all cache variables with
...
Get the video and create a result folder to save images. Next run the following command:

```sh
./extractFrames dil_calib.mp4 dilImgs1
```
...
### Enable High Performace on Linux Machine
